...
The commit type determines which CHANGELOG section the change lands in
when git-cliff regenerates the file at release time:

| Type | CHANGELOG section |
| --- | --- |
| `feat` | `### Added` |
| `fix` | `### Fixed` |
| `perf` / `refactor` / `style` / `revert` / `docs` | `### Changed` |
| `chore` / `ci` / `build` / `test` | not included |

The commit *subject* (minus the type/scope prefix) becomes the bullet
text verbatim, so phrase the subject as the changelog entry you would
want to read.

## CHANGELOG

`CHANGELOG.md` is regenerated by [git-cliff](https://git-cliff.org/)
from commit history; do **not** hand-edit it. Workflow:

- Write a well-formed Conventional Commit subject — that *is* the
  changelog entry (see the table in the Commits section above).
- Preview what an unreleased section would look like with
  `mise run changelog`.
- At release time `scripts/release-prep.py` runs `git-cliff` to
  regenerate the full file, bumps `Cargo.toml`, and commits both.

The 0.1.0 section predates Conventional Commits and is preserved
verbatim inside `cliff.toml`'s `footer` block. Do not delete or rewrite
it.
