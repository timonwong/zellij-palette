...
Project-specific guidance for agents working in this repo. Falls back to
`/workspaces/timon-workspace/CLAUDE.md` and the user-global CLAUDE.md for
cross-repo conventions (Chinese conversation, English commits/PRs,
Markdown sanitization rules, Mermaid version handling, …).
...
- No "guess" / placeholder functions that return a hard-coded constant
  dressed up as dynamic state — the previous `last_rendered_rows_guess`
  was a footgun.
- No unused struct fields kept "for later" — the user explicitly bounced
  `last_cols` on this principle.
...
`CHANGELOG.md` follows Keep-a-Changelog. Update the `[Unreleased]`
section in the same commit that introduces a user-visible change. Cut a
version section only when bumping `Cargo.toml`'s `version`.
